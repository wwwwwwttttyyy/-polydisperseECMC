...
struct Parameters {
    ...
    long long sample_interval_snapshot = 0;  // 0 means snapshots are disabled
...
void printUsage(const char* progName) {
    ...
    std::cout << "  -O <dir>       Output directory (all results will be written here)\n";
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            // Read the parameter file immediately
            ...
            params.outputDir = std::string(argv[++i]);
        } else {
    ...
    // If no RNG seed specified, use a random value
    ...
    try {
        ...
        std::cout << "  Radius: mean = " << sys.radiusMean 
                  << ", max = " << sys.radiusMax 
                  << ", min = " << sys.radiusMin << "\n";
        std::cout << "  Packing fraction = " << sys.getPackingFraction() << "\n\n";
    ...
    if (sys.checkOverlap(0.1)) { // Use a reasonable overlap threshold, e.g., 0.1
    ...
    algo_para.set_outputDir(params.outputDir);
    ...
    try {
        ...
        sys.saveConfig("initialconf.dat");  // Save initial configuration backup
        engine.run();
    ...
    if (sys.checkOverlap(0.1)) {
        std::cerr << "Error: Final configuration has overlaps!\n";
    }
    std::string outputDir = algo_para.get_outputDir();
    std::string finalConf = outputDir + "/" + "finalconf.dat";
    // Save final configuration
    try {
        std::cout << "Saving final configuration to " << finalConf << "...\n";
        if (params.binary_output) {
            sys.saveConfigBinary(finalConf);
        } else {
            sys.saveConfig(finalConf);
        }
        std::cout << "Done!\n\n";
