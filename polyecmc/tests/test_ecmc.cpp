 ...
 * Example: 256 hard disks, square lattice → liquid equilibration
 */
...
void generateSquareLattice(System& sys, int nx, int ny, double phi, double polydispersity = 0.0, unsigned int seed = 123) {
    ...
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < ny; ++j) {
            ...
            sys.radius[idx] = dist(rng);  // Polydisperse radius
            ++idx;
        }
    }
    ...
    std::cout << "  Box: " << Lx << " x " << Ly << ", φ = " << phi << "\n";
    if (polydispersity > 0) {
        std::cout << "  Polydispersity: ±" << (polydispersity * 100) << "% "
                  << "(r ∈ [" << r_mean * (1.0 - polydispersity) << ", " 
                  << r_mean * (1.0 + polydispersity) << "])\n";
...
int main() {
    ...
    const unsigned int rng_seed = 42;  // Fixed random number seed
    const double polydispersity = 0.02;  // Very weak polydispersity: ±2% (radius 0.98-1.02)
    ...
    // Setup ECMC (using auto-generated cell grid)
    std::cout << "========== Running ECMC ==========\n";
    
    AlgorithmPara algo_para;
    algo_para.set_n_steps(n_chains);
    algo_para.set_chain_length(1.0);
    algo_para.set_sample_interval_pressure(100000);  // Output cumulative average every 100,000 chains
    algo_para.set_sample_interval_snapshot(100000);  // Output binary snapshot every 100,000 chains
    algo_para.set_rng_seed(rng_seed);  // Set random number seed
    algo_para.set_n_equilibration_pressure(100000);  // First 100,000 chains for pressure equilibration, then clear data
