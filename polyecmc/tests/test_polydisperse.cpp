...
// Inverse power law distribution sampling for radii
// P(r) ∝ r^(-α), here using α = 3
...
void generatePolydisperseSquareLattice(System& sys, int nx, int ny, 
    ...
    // Ensure lattice constant is large enough to avoid maximum particle overlap
...
int main() {
    ...
    // Parameter settings - stress test
    double rmin = 0.8;
    double rmax = 1.1;
    ...
    AlgorithmPara algo;
    algo.set_n_steps(5000000);       // Run 5 million chains (stress test)
    ...
    // Statistics for radius distribution
