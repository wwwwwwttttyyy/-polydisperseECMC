 ...
 *     Formula: β P = ρ + (N/V) * Σ(deltaIJ) / L_total
 *             = ρ * (1 + Σ(deltaIJ) / L_total)
 *     Where: deltaIJ = dx_ij - L_event (collision momentum transfer)
 *            L_total = Σ(chain_length) (total chain length)
...
public:
    ...
    /**
     * @brief Calculate pressure factor (1 + Σ deltaIJ / L_total)
    ...
    /**
     * @brief Calculate reduced pressure X component
     */
    ...
    /**
     * @brief Calculate reduced pressure Y component
     */
