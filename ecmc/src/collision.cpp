...
static const double EPSILON = 1e-12;  // Several orders larger than machine precision
    ...
    double maxDistance)
{
    ...
    grid.forEachNeighbor(activeCellID, activeSlotK, xDirections,
        [&](int targetCellID, int targetSlotK, double dx, double dy, double targetRadius) {
            ...
            // 2. Fast rejection: dx must be positive (target ahead)
            ...
            if (eventDist < 0.0) {
                return;  // Negative distance is physical error (particles already overlap), skip
            }
            
            // 5. INF means never collide (too far in perpendicular direction)
    ...
    double maxDistance)
{
    ...
    grid.forEachNeighbor(activeCellID, activeSlotK, yDirections,
        [&](int targetCellID, int targetSlotK, double dx, double dy, double targetRadius) {
            ...
            // 2. Fast rejection: dy must be positive (target ahead)
            if (dy <= EPSILON) {
                return;
            }
            ...
            if (eventDist < minEvent.distance) {
                ...
                minEvent.dxInitial = dx;
                minEvent.dyInitial = dy;
                // deltaIJ = dy - eventDist (Y direction contact distance at collision moment)
