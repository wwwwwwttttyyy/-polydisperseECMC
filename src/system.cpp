...
bool System::checkOverlap(double overlapTolerance) const {
    size_t N = x.size();
...
void System::applyPBC(size_t i) {
    ...
    x[i] = apply_pbc(x[i], boxsize[0]);
    y[i] = apply_pbc(y[i], boxsize[1]);
}
...
std::array<double, 2> System::displacement(size_t i, size_t j) const {
    ...
    double dx = x[j] - x[i];
    double dy = y[j] - y[i];
    dx = apply_pbc(dx, boxsize[0]);
    dy = apply_pbc(dy, boxsize[1]);
...
void System::loadConfig(const std::string& filename) {
    ...
    while (ifs >> x_val >> y_val >> r_val) {
        x_val = apply_pbc(x_val, boxsize[0]);
        y_val = apply_pbc(y_val, boxsize[1]);
        addParticle(x_val, y_val, r_val);
    }
    ifs.close();

    // 小幅随机扰动，避免精确重合（固定种子便于复现）
    ...
    std::cout << "Loaded " << size() << " particles from " << filename 
              << " (PBC applied, box centered at origin, boxsize+epsilon)\n";
}
